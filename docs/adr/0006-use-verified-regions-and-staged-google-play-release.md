...
The daily core journey, measured from starting ingredient confirmation to choosing a dinner and excluding first-time setup or artifact download time, has a five-minute product target under the documented test network conditions. First-time setup should require no more than five minutes of active user interaction when model download waiting time is excluded. Closed testing records misses and uses them to prioritize usability or performance work, but missing either time target alone does not block release.
