...
The daily core journey, measured from starting ingredient confirmation to choosing a dinner and excluding first-time setup or artifact download time, must be completable within five minutes under the documented test network conditions. First-time setup should require no more than five minutes of active user interaction when model download waiting time is excluded.
